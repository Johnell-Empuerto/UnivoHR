...
const EVAL_COMPLETED = `LOWER(eke.status) IN ('submitted', 'completed', 'approved', 'hr_approved')`;

const getPerformanceSummary = async (employeeId) => {
  const result = await pool.query(
    `SELECT
       (SELECT final_score FROM employee_kpi_evaluations eke
        WHERE eke.employee_id = $1 AND ${EVAL_COMPLETED}
        ORDER BY eke.created_at DESC LIMIT 1) AS latest_score,
       (SELECT ROUND(AVG(final_score), 2) FROM employee_kpi_evaluations eke
        WHERE eke.employee_id = $1 AND ${EVAL_COMPLETED}) AS average_score,
       (SELECT COUNT(*) FROM employee_kpi_evaluations eke
        WHERE eke.employee_id = $1 AND ${EVAL_COMPLETED}) AS completed_count,
       (SELECT COUNT(*) FROM employee_kpi_evaluations eke
        WHERE eke.employee_id = $1 AND NOT ${EVAL_COMPLETED}) AS pending_count,
