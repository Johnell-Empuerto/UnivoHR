...
import {
  ...
  getRawLogs, getRawLogById,
  getMappings, createMapping, updateMapping, deleteMapping,
  getEmployeeDeviceUsers, createEmployeeDeviceUser, updateEmployeeDeviceUser, deleteEmployeeDeviceUser,
  searchEmployees,
} from "@/services/deviceIntegrationService";
import { getActiveBranches } from "@/services/branchService";
import type {
  Device, RawLog, DeviceLogMapping, EmployeeDeviceUser,
} from "@/services/deviceIntegrationService";
...
import {
  Monitor, List, Users, GitBranch, Plus, Trash2, Edit, Eye,
  ChevronLeft, ChevronRight, RefreshCw, Server, Search, X,
} from "lucide-react";
      ...
      <CardContent>
        <Tabs value={activeSubTab} onValueChange={setActiveSubTab}>
          <TabsList className="flex w-full gap-2 overflow-x-auto mb-6">
            <TabsTrigger value="devices" className="flex items-center gap-2 flex-1" disabled={!canViewDevices}>
              <Monitor className="h-4 w-4" /> Devices
            </TabsTrigger>
            <TabsTrigger value="logs" className="flex items-center gap-2 flex-1" disabled={!canViewLogs}>
              <List className="h-4 w-4" /> Raw Logs
            </TabsTrigger>
            <TabsTrigger value="device-users" className="flex items-center gap-2 flex-1" disabled={!canViewDevices}>
              <Users className="h-4 w-4" /> Device User Mapping
            </TabsTrigger>
            <TabsTrigger value="mappings" className="flex items-center gap-2 flex-1" disabled={!canViewDevices}>
...
function DevicesTab({ canManage }: { canManage: boolean }) {
  ...
  const [branches, setBranches] = useState<{ id: number; name: string; timezone: string }[]>([]);
  const [form, setForm] = useState({
    name: "", type: "BIOMETRIC", serial_number: "", model: "",
    ip_address: "", port: "", location: "", status: "ACTIVE",
    api_key: "", notes: "", branch_id: "",
  });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ name: "", type: "BIOMETRIC", serial_number: "", model: "", ip_address: "", port: "", location: "", status: "ACTIVE", api_key: "", notes: "", branch_id: "" });
    setShowDialog(true);
  };
  ...
  const openEdit = (d: Device) => {
    setEditing(d);
    setForm({
      name: d.name, type: d.type, serial_number: d.serial_number || "",
      model: d.model || "", ip_address: d.ip_address || "",
      port: d.port ? String(d.port) : "", location: d.location || "",
      status: d.status, api_key: d.api_key || "", notes: d.notes || "",
        ...
        <div className="rounded-md border">
          <Table>
            ...
            <TableBody>
              {devices.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={canManage ? 8 : 7} className="text-center py-8 text-muted-foreground italic">
                    No devices configured.
                  </TableCell>
                </TableRow>
              ) : devices.map((d) => (
                <TableRow key={d.id} className="border-b border-gray-400/50 dark:border-gray-400/50">
                  ...
                  <TableCell>
                    {d.branch_name ? (
                      <span className="text-xs" title={d.branch_timezone || ""}>
                        {d.branch_name}{d.branch_timezone ? ` (${d.branch_timezone})` : ""}
                      </span>
                    ) : (
                  ...
                  <TableCell>
                    <Badge variant={d.status === "ACTIVE" ? "default" : d.status === "OFFLINE" ? "secondary" : "destructive"}>
                      {d.status}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <span className="text-xs text-muted-foreground">
                      ...
                      {(d.pending_logs ?? 0) > 0 && (
                        <span className="ml-1 text-amber-500">({d.pending_logs} pending)</span>
                      )}
                    </span>
                  </TableCell>
                  {canManage && (
                    <TableCell>
                      <div className="flex gap-1">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleDelete(d.id)} className="text-red-500">
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Configure the device connection details.</DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <Label>Device Name *</Label>
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Main Entrance Biometric" />
            </div>
            <div>
              <Label>Type</Label>
            ...
            <div>
              <Label>Model</Label>
              <Input value={form.model} onChange={(e) => setForm({ ...form, model: e.target.value })} placeholder="Optional" />
            </div>
            <div>
            ...
            <div>
              <Label>Port</Label>
              <Input value={form.port} onChange={(e) => setForm({ ...form, port: e.target.value })} placeholder="e.g. 4370" />
            </div>
            <div className="col-span-2">
            ...
            <div>
              <Label>API Key</Label>
              <Input value={form.api_key} onChange={(e) => setForm({ ...form, api_key: e.target.value })} placeholder="For device authentication" />
            </div>
            <div className="col-span-2">
              <Label>Notes</Label>
...
function RawLogsTab() {
  ...
  useEffect(() => {
    getDevices({ limit: 100 }).then((res) => setDevices(res.data)).catch(() => {});
  }, []);
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      const res = await getRawLogs({
        ...
        device_id: deviceFilter && deviceFilter !== "all" ? parseInt(deviceFilter) : undefined,
  ...
  const statusBadge = (status: string) => {
    const variants: Record<string, "default" | "secondary" | "destructive" | "outline"> = {
      PENDING: "outline", PROCESSED: "default", FAILED: "destructive", DUPLICATE: "secondary",
    };
  ...
  const hasActiveFilters = statusFilter || deviceFilter || sourceFilter || dateFrom || dateTo;
            ...
            <TableBody>
              {logs.length === 0 ? (
                ...
                <TableRow key={log.id} className="border-b border-gray-400/50 dark:border-gray-400/50">
                  <TableCell>{log.device_name || `Device #${log.device_id}`}</TableCell>
                  <TableCell className="font-mono text-xs">{log.employee_code || "—"}</TableCell>
                  ...
                  <TableCell>
                    <Button variant="ghost" size="sm" onClick={() => viewPayload(log.id)}>
      ...
      <Dialog open={payloadDialog} onOpenChange={setPayloadDialog}>
        <DialogContent className="!max-w-5xl w-[95vw] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Original device payload received for this log entry.</DialogDescription>
          </DialogHeader>
          {payloadLoading ? <Loader /> : payloadLog && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div><span className="text-muted-foreground">Log ID:</span> {payloadLog.id}</div>
                <div><span className="text-muted-foreground">Device:</span> {payloadLog.device_name || `Device #${payloadLog.device_id}`}</div>
                <div><span className="text-muted-foreground">Employee Code:</span> {payloadLog.employee_code || "—"}</div>
                <div><span className="text-muted-foreground">Timestamp:</span> {new Date(payloadLog.timestamp).toLocaleString()}</div>
                <div><span className="text-muted-foreground">Source:</span> {payloadLog.source}</div>
                <div><span className="text-muted-foreground">Status:</span> {payloadLog.status}</div>
              </div>
              <div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setPayloadDialog(false)}>Close</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function DeviceUserMappingTab({ canManage }: { canManage: boolean }) {
  ...
  const [form, setForm] = useState({
    employee_id: 0, device_id: "", device_user_id: "", active: true,
  });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ employee_id: 0, device_id: "", device_user_id: "", active: true });
  ...
  const openEdit = (m: EmployeeDeviceUser) => {
    setEditing(m);
    setForm({
      employee_id: m.employee_id, device_id: String(m.device_id),
      device_user_id: m.device_user_id, active: m.active,
    });
    setSelectedEmployee({
      id: m.employee_id,
  ...
  const handleSave = async () => {
    ...
    if (!form.device_id) { toast.error("Select a device"); return; }
    if (!form.device_user_id.trim()) { toast.error("Device User ID is required"); return; }
    try {
      const payload = {
          ...
          <Table>
            ...
            <TableBody>
              {filteredMappings.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={canManage ? 6 : 5} className="text-center py-8 text-muted-foreground italic">
                    {search ? "No matching mappings found." : "No device user mappings configured."}
                  </TableCell>
                </TableRow>
              ) : filteredMappings.map((m) => (
                <TableRow key={m.id} className="border-b border-gray-400/50 dark:border-gray-400/50">
                  ...
                  <TableCell className="font-mono text-xs">{m.device_user_id}</TableCell>
                  <TableCell>
                    <Badge variant={m.active ? "default" : "secondary"} className={m.active ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : ""}>
                  ...
                  {canManage && (
                    <TableCell>
                      <div className="flex gap-1">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleDelete(m.id)} className="text-red-500">
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </div>
                    </TableCell>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="!max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Map a device user ID to an employee record.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4">
              ...
              {selectedEmployee ? (
                <div className="flex items-center justify-between border rounded-md px-3 py-2 mt-1">
                  <div className="text-sm">
                    <span className="font-mono text-xs font-medium">{selectedEmployee.employee_code}</span>
                    {" — "}
                  ...
                  <Button variant="ghost" size="sm" onClick={() => { setSelectedEmployee(null); setForm((prev) => ({ ...prev, employee_id: 0 })); }} className="h-6 w-6 p-0">
            ...
            <div>
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">The user identifier from the device (e.g. fingerprint ID, RFID tag, username).</p>
            </div>
            <div className="flex items-center gap-2">
              <Switch
                checked={form.active}
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full border rounded pl-7 pr-7 py-1.5 text-sm bg-background"
              />
              {empSearch && (
                <X
...
function MappingsTab({ canManage }: { canManage: boolean }) {
  ...
  const [form, setForm] = useState({
    device_id: "", field_source: "", field_target: "", transform_expression: "",
  });

  const TARGET_FIELDS = ["employee_code", "timestamp", "event_type", "device_id", "employee_id"];
  const SOURCE_FIELDS = [
    "employee_code", "employeeCode", "employee_id", "employeeId",
    "user_id", "userId", "card_number", "cardNumber",
    "rfid_tag", "rfidTag", "fingerprint_id", "fingerprintId",
    "face_id", "faceId", "pin",
    "timestamp", "date_time", "datetime", "time", "log_time",
    "event_type", "eventType", "event", "status",
    "device_id", "deviceId",
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [m, d] = await Promise.all([
        getMappings(deviceFilter && deviceFilter !== "all" ? parseInt(deviceFilter) : undefined),
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ device_id: "", field_source: "", field_target: "", transform_expression: "" });
    setShowDialog(true);
  };
  ...
  const openEdit = (m: DeviceLogMapping) => {
    setEditing(m);
    setForm({
      device_id: String(m.device_id), field_source: m.field_source,
      field_target: m.field_target, transform_expression: m.transform_expression || "",
    });
    setShowDialog(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await updateMapping(editing.id, { ...payload, is_active: editing.is_active });
        toast.success("Mapping updated");
      } else {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <Select value={deviceFilter} onValueChange={(v) => { setDeviceFilter(v); }}>
            <SelectTrigger className="w-56"><SelectValue placeholder="All Devices" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All Devices</SelectItem>
              {devices.map((d) => (
                <SelectItem key={d.id} value={String(d.id)}>{d.name}</SelectItem>
          ...
          <Table>
            ...
            <TableBody>
              {mappings.length === 0 ? (
                ...
                <TableRow key={m.id} className="border-b border-gray-400/50 dark:border-gray-400/50">
                  <TableCell>{deviceName(m.device_id)}</TableCell>
                  <TableCell className="font-mono text-xs">{m.field_source}</TableCell>
                  <TableCell className="font-mono text-xs">{m.field_target}</TableCell>
                  <TableCell className="text-xs text-muted-foreground">{m.transform_expression || "—"}</TableCell>
                  <TableCell>
                    <Badge variant={m.is_active ? "default" : "secondary"} className={m.is_active ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : ""}>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="!max-w-3xl w-full max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Mapping" : "Add Mapping"}</DialogTitle>
            <DialogDescription>Map a source field from the device payload to a system field.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4">
            <div>
              <Label>Device *</Label>
            ...
            <div>
              ...
              <Select value={form.field_source} onValueChange={(v) => setForm({ ...form, field_source: v })}>
                <SelectTrigger><SelectValue placeholder="Select source field" /></SelectTrigger>
                <SelectContent>
                  {SOURCE_FIELDS.map((f) => (
              ...
              <p className="text-xs text-muted-foreground mt-1">Field name in the device payload</p>
            </div>
            <div>
              <Label>Target Field *</Label>
              <Select value={form.field_target} onValueChange={(v) => setForm({ ...form, field_target: v })}>
            ...
            <div>
              ...
              <Input value={form.transform_expression} onChange={(e) => setForm({ ...form, transform_expression: e.target.value })} placeholder="e.g. trim, uppercase" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowDialog(false)}>Cancel</Button>
