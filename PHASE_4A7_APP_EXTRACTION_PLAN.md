...
// Public health check
app.get("/api/health", (req, res) => { /* same as today */ });
...
// Graceful shutdown
const shutdown = async (signal) => { /* same as today */ };
process.on("SIGTERM", () => shutdown("SIGTERM"));
process.on("SIGINT", () => shutdown("SIGINT"));

// DB connection + admin seed
const pool = require("./config/db");
pool.connect().then(async () => { /* same as today */ }).catch(/* ... */);
...
jest.mock("../services/deviceProcessing.queue", () => ({
  deviceProcessingQueue: { process: jest.fn(), on: jest.fn(), close: jest.fn().mockResolvedValue() },
